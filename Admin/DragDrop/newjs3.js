$(document).ready(function () {
	...
	*****************************/
    ...
    var dropZoneId = "drop-zones";
    var buttonId = "clickHeres";
    var mouseOverClass = "mouse-over";
    defaultUploadBtn = $('#filedrgdrops');
    var dropZone = $("#" + dropZoneId);
    var ooleft = dropZone.offset().left;
    var ooright = dropZone.outerWidth() + ooleft;
    var ootop = dropZone.offset().top;
    var oobottom = dropZone.outerHeight() + ootop;
    var inputFile = dropZone.find("#filedrgdrop1");
    ...
    var processFiles = function (files) {
        if (files && typeof FileReader !== "undefined") {
            ...
            for (var i = 0; i < files.length; i++) {
                readFile(files[i]);
                $('#ctl00_ContentPlaceHolder1_hd1').val(i);
            }
        } else {
	...
	*****************************/
    var createPreview = function (file, newURL) {
        ...
        var filenames = file.name.substr(0, file.name.lastIndexOf('.'));
        var NameFile = file.name.substr(0, file.name.lastIndexOf('.')); // hardik add
        NameFile = NameFile.substr(0, 13);                              // hardik add

        var imageObj = {};
        imageObj.filePath = newURL;
        //imageObj.fileName = file.name.substr(0, file.name.lastIndexOf('.')); //subtract file extension       // hardik comment
        imageObj.fileName = NameFile; //subtract file extension // hardik add
        ...
        imageObj.fileNamewithoutspace1 = filenames.replace(/\s/g, '');
        //extend filename
        var effect = $('input[name=effect]:checked').val();
        if (effect == 'grayscale') {
        ...
        var img = $("#imageTemplate1").tmpl(imageObj).prependTo("#result1")
		.css({
		    'z-index': zindex++
		})
		.show();
	...
	****************************/
    var uploadToServer = function (oldFile, newFile) {
        var h = $('#ctl00_ContentPlaceHolder1_hd1').val();
        // prepare FormData
    ...
    var processFileInIE = function (file) {
        ...
        if (filepath) {
            ...
            if ($.inArray(fileext, extension) > -1) {
                ...
                $("#imageTemplate1").tmpl(imageObj).prependTo("#result1")
				.hide()
				.css({
				    'z-index': zindex++
				})
				.show();
                $('#result1').find('figcaption span').hide();
            } else {
                $('#err').text('*Selected file format not supported!');
            }
        }
