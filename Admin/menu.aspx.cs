...
public partial class Admin_menu : System.Web.UI.Page
{
    ...
    {
        Page.Title = "Admin Menu List - " + System.Configuration.ConfigurationManager.AppSettings["AminPageTitle"];
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["flag"] == "add")
            {
                //trmsg.Visible = true;
                lblmsg.Visible = true;
                lblmsgs.Text = "Admin Menu has been added successfully";
            }
            else if (Request.QueryString["flag"] == "edit")
            {
                // trmsg.Visible = true;
                lblmsg.Visible = true;
                lblmsgs.Text = "Admin Menu has been updated successfully";
            }
            else if (Request.QueryString["flag"] == "delete")
            {
                //trmsg.Visible = true;
                lblmsg.Visible = true;
                lblmsgs.Text = "Admin Menu has been deleted successfully";
            }
    ...
    protected void BindContents(string search = "")
    {
        ...
        {
            pageSize = 50;
            objMenu.title = Server.HtmlEncode(txtsearch.Text.Trim());
            if (pageNo == 0) { pageNo = 1; }
            objMenu.pageNo = pageNo;
            objMenu.pageSize = pageSize;
            objMenu.SortExpression = SortExpression;
            querystring = "&key=" + txtsearch.Text;
            //querystring = "&pageSize=" + ddlpageSize.SelectedValue + "&key=" + txtsearch.Text;
            dtcontents = objMenu.SearchItem();
            totalrecs = objMenu.TotalRecord;
            if (pageNo == 1)
            {
                objMenu.pageNo = 1;
                pageNo = 1;
            ...
            {
                objMenu.pageNo = (pageNo - 1) * pageSize;
            }
            objMenu.pageSize = pageSize;
            ...
            String strpaging = CommonFunctions.AdminPaging(totalpages, pageNo, querystring, "menu.aspx");
    ...
    protected void lnkStatus_click(object sender, EventArgs e)
    {
        ...
        objMenu.isactive = Convert.ToByte(Convert.ToInt32(((LinkButton)sender).CommandArgument) == 0 ? 1 : 0);

        objMenu.idmenu = Convert.ToInt32(gvAdmin.DataKeys[gvAdmin.Rows[row.RowIndex].RowIndex].Value.ToString());
        objMenu.UpdateStatus();
        BindContents();
        //trmsg.Visible = true;
    ...
    protected void imgbtnDelete_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < gvAdmin.Rows.Count; i++)
            {
                ...
                {
                    con += 1;
                    objMenu.idmenu = Convert.ToInt32(gvAdmin.DataKeys[gvAdmin.Rows[i].RowIndex].Value.ToString());
            ...
            //Response.Redirect("menu.aspx?flag=delete&key=" + txtsearch.Text + "  &pageSize=" + ddlpageSize.SelectedValue + "");
            Response.Redirect("menu.aspx?flag=delete&key=" + txtsearch.Text + "");
        }
        catch (Exception ex) { throw ex; }
